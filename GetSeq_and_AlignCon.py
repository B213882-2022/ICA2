...
while True:
    ...
    while True:  # Use exit sequences? or get from Enterz?
        ...
        if selection == 'yes' or len(selection) == 0:
            ...
            while True:
                family_name = input('\nEnter your protein family name: (e.g. Pyruvate dehydrogenase)\
                                     \n(Enter "exit" to quit programme)\n').strip()
        ...
        if r.returncode == 0:
            if subprocess.getoutput(r'wc -l < %s' % (fa_dir)) == '0':
                while True:
                    ...
                    if r == 'yes' or len(r) == 0:
                        subprocess.run(r'rm -rf %s' % (fa_dir),shell=True)
                        break
                    elif r == 'no':
                        print('\nGoodbye!')
                        subprocess.run(r'rm -rf %s' % (fa_dir),shell=True)
    ...
    if select_code == 2:  # check sequences validity
        ...
        while True:
            ...
            if r == 'yes':
                ...
                print('\nGoodbye!')
                subprocess.run(r'rm -rf %s' % fa_dir)
                sys.exit()
            else:
...
while True:
    ...
    while True:  # ask from user
        ...
        if r == 'yes':
            print(pd.concat([seq_df[['protein_id','species','seq_length']],seq_df['sequence'].apply(lambda x: x[:10]+"...")],axis=1).sort_values(by='species'))
            break
        elif r == 'no':
            break
        elif len(r) == 0:
...
if r.returncode == 0:
    print('\nSuccessful! The plot can be found at: %s'%wd)
else:
    print('\nSomething goes wrong...' + '\nReturn code: ' + str(r.returncode))
    print('Wrong command: ' + r.args)
    sys.exit()
